...
// Team avatars come from data/avatars/{filename}.png, generated by
// scripts/generate_avatars.py via the generate-avatars GHA workflow.
// data/avatars/manifest.json maps displayName -> filename. Until a name has
// a bespoke avatar generated, we render an initials-in-a-colored-circle SVG
// placeholder so the layout stays consistent.
let teamAvatarManifest = {};

function teamAvatarPlaceholder(name) {
  const safe = name || "?";
  const initials =
    safe
      .split(/\s+/)
      ...
      .toUpperCase()
      .slice(0, 2) || "?";
  // Deterministic hue from the name so the same team always gets the same
  // colored placeholder.
  let hash = 0;
  ...
  const svg =
    ...
    `<circle cx="32" cy="32" r="30" fill="hsl(${hue},55%,50%)"/>` +
...
function teamAvatar(name) {
  if (!name) return null;
  const filename = teamAvatarManifest && teamAvatarManifest[name];
  if (!filename) return teamAvatarPlaceholder(name);

  const img = document.createElement("img");
  img.className = "team-avatar";
  img.src = `data/avatars/${filename}`;
  img.alt = "";
  img.loading = "lazy";
  // If the manifest knows about a file but the file is missing on disk
  // (mid-deploy race), swap in the placeholder rather than showing a broken
  // image icon.
  img.onerror = function () {
    const ph = teamAvatarPlaceholder(name);
...
async function main() {
  ...
  let scores, entriesData, avatarManifest;
  try {
    [scores, entriesData, avatarManifest] = await Promise.all([
      loadJson("data/scores.json"),
      loadJson("data/entries.json").catch(() => ({ entries: [] })),
      loadJson("data/avatars/manifest.json").catch(() => ({ avatars: {} })),
    ]);
  } catch (e) {
  ...
  teamAvatarManifest = (avatarManifest && avatarManifest.avatars) || {};
